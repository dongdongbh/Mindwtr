...
11. Desktop File Sync durably journals an exact, lease-bound scratch path before returning it to the renderer. Recovery runs after acquiring the same folder lease and removes only that journal-owned regular file; it never scans the shared folder by filename pattern. Android path publication uses the native generation installer for create-no-replace. Generic Android SAF cannot atomically create and fill a document, so a process death during that provider operation can leave a corrupt target; later runs preserve it and fail closed rather than overwrite it.
