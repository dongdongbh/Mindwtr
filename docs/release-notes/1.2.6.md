...
- Fresh self-hosted web sessions can prefill the Cloud URL from the same origin or `MINDWTR_DEFAULT_CLOUD_URL` without overwriting an existing choice.
...
- Cloud deployments can read `MINDWTR_CLOUD_AUTH_TOKENS` from a protected file.
...
- The Arch beta package is now `mindwtr-beta-bin`; the legacy `mindwtr-bin-beta` package remains available during the documented manual migration period.
