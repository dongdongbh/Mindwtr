 ...
 * function is called. Across 2411 matching mobile call sites it produced no false
 * positives; single-argument lookalikes that are not i18n (an `'calendar.fill'` or
 * `'trash.fill'` SF Symbol name) never pair with a prose fallback and so never
 * match.
 */
...
function collectKeyReferences(): Reference[] {
  ...
  for (const path of collectSourceFiles(APP_ROOT)) {
