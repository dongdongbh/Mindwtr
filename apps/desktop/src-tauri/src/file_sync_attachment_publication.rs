...
const JOURNAL_DIR_NAME: &str = "file-sync-attachment-publications-v1";
const JOURNAL_ENTRY_PREFIX: &str = "publication-v1-";
...
{
    let _guard = journal_lock();
    let (path, entry) = read_owned_entry(data_dir, sync_root, operation_id)?;
    let outcome = publish(
        &entry.scratch_path,
        &entry.target_path,
        entry.expected_size,
...
pub(crate) fn abandon(data_dir: &Path, sync_root: &Path, operation_id: &str) -> Result<(), String> {
    let _guard = journal_lock();
    let (path, entry) = read_owned_entry(data_dir, sync_root, operation_id)?;
...
pub(crate) fn recover(data_dir: &Path, sync_root: &Path) -> Result<usize, String> {
    let _guard = journal_lock();
    let directory = journal_dir(data_dir);
    validate_journal_dir(&directory)?;
...
mod tests {
    ...
    fn collision_keeps_the_owned_scratch_until_explicit_abandon() {
        ...
        abandon(&data_dir, &sync_root, &reservation.operation_id).expect("abandon");
        assert!(!scratch.exists());
        assert!(!entry_path(&data_dir, &sync_root, &reservation.operation_id).exists());
    }
    ...
    fn another_root_cannot_abandon_or_recover_an_owned_scratch() {
        ...
        assert_eq!(recover(&data_dir, &other_root).expect("other recovery"), 0);
        assert!(scratch.exists());
        assert!(abandon(&data_dir, &other_root, &reservation.operation_id).is_err());
        assert!(scratch.exists());
    }
    ...
    fn symlink_replacement_fails_closed_without_touching_its_target() {
        ...
        let error = recover(&data_dir, &sync_root).expect_err("must fail closed");
    ...
    fn malformed_matching_journal_fails_closed_without_scanning_shared_files() {
        ...
        assert!(recover(&data_dir, &sync_root).is_err());
