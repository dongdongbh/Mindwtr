...
Implemented with a private initial mapping inventory and the existing fresh ending sweep. Both 5,000-task regressions failed with 5,000 point lookups before the change and pass with zero afterward. Core/calendar scheduler 23 tests, desktop adapter 12, three platform typechecks, scoped lint, and diff checks passed. The first isolated mobile run resolved shared core to the preintegration main checkout through its dependency symlink; it did not validate the new combination. The integrated suite exposed ten point-only mobile mock failures. The mobile fixtures now model persisted inventory and explicitly assert full runs avoid point lookups; focused and aggregate results are recorded with the review. Full verification and independent closure are recorded in the local review result.
